    ...
    for i in range(1, 375):
        for j in range(1, 389):
            ...
            if a[i][j] == 1:
                ...
                if (feature_vector['type'] == 'bifurcation_point' or feature_vector['type'] == 'ridge_ending') and (feature_vector['orientation'] != 0 and feature_vector['orientation'] != 360and feature_vector['orientation'] != -1):
...
def crossing_number_algorithm(img, file1, file2):
    ...
    for i in range(375):
        for j in range(389):
            a[0][j] = 0
            a[i][0] = 0
    collection, isolate_point, ridge_ending, continuing_ridge, bifurcation_point, crossing_point, valid_bifurcation_point, valid_ridge_ending = crossing_num_calc(a)
    ...
    pixel_type_collection = {"Isolated Points": isolate_point, "Ridge Ending Points": ridge_ending,
                             ...
                             "Crossing Points": crossing_point, "Total Points": sum_all, "Valid Bifurcation Points": valid_bifurcation_point, "Valid Ridge Ending Points": valid_ridge_ending}
    # print(collection)
