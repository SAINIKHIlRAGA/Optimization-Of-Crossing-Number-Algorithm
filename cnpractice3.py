...
def calc_i(i, a, collection, c):
    isolate_point = c["isolate_point"]
    ridge_ending = c["ridge_ending"]
    continuing_ridge = c["continuing_ridge"]
    bifurcation_point = c["bifurcation_point"]
...
def crossing_num_calc_parallel(a):
    collection = []
    continuing_ridge = 0
    bifurcation_point = 0
    isolate_point = 0
    ...
    c["isolate_point"] = 0
    c["ridge_ending"] = 0
    c["continuing_ridge"] = 0
    c["bifurcation_point"] = 0
    c["crossing_point"] = 0

    res = [p.apply(calc_i, args=(i, a, collection, c)) for i in l]
    for i in res:
        for j in i:
            collection.append(j)
    p.close()
...
def crossing_num_calc_serial(a):
    collection = []
    feature_vector = {}
    isolate_point = 0
    ridge_ending = 0
...
def crossing_number_algorithm_parallel(img, file1):
    ...
    a = two_dim(new_pix1)
    collection, isolate_point, ridge_ending, continuing_ridge, bifurcation_point, crossing_point = crossing_num_calc_parallel(a)
...
def crossing_number_algorithm_serial(img, file1):
    img = Image.open(img, 'r')
    pix_vals = list(img.getdata())
    pix_vals = thresholding(pix_vals)
    new_pix = pix_vals
    new_pix1 = padding(new_pix)
    a = two_dim(new_pix1)
    collection, isolate_point, ridge_ending, continuing_ridge, bifurcation_point, crossing_point = crossing_num_calc_serial(a)
    sum_all = isolate_point + ridge_ending + continuing_ridge + bifurcation_point + crossing_point
    pixel_type_collection = {"Isolated Points": isolate_point, "Ridge Ending Points": ridge_ending,
                             "Ridge Continuing Points": continuing_ridge, "Bifurcation Points": bifurcation_point,
                             "Crossing Points": crossing_point, "Total Points": sum_all}
...
if __name__ == '__main__':
    ...
    img1 = "101_1_skel.tif"
    file11 = "101_1_Feature_Vector_Parallel.txt"
    file12 = "101_1_Feature_Vector_Serial.txt"
    ...
    start1 = time.time()
    processes = []
    process1 = Process(target=crossing_number_algorithm_parallel, args=(img1, file11,))
    process2 = Process(target=crossing_number_algorithm_parallel, args=(img1, file21,))
    process3 = Process(target=crossing_number_algorithm_parallel, args=(img1, file31,))
    process4 = Process(target=crossing_number_algorithm_parallel, args=(img1, file41,))
    ...
    start2 = time.time()
    crossing_number_algorithm_serial(img1, file12,)
    crossing_number_algorithm_serial(img1, file12, )
    crossing_number_algorithm_serial(img1, file12, )
    crossing_number_algorithm_serial(img1, file12, )
