    ...
    for i in collection3:
        for j in collection4:
            ...
            if i['orientation'] == j['orientation']:
                matched += 1
                x = i['position_x'] - j['position_x']
                x *= x
                y = i['position_y'] - j['position_y']
                y *= y
                dist = math.sqrt(x + y)
                print(dist, "---------------------")
                    # min_dict_1 = {}
                    # min_dict_2 = {}
                    # for l in range(collection3.__len__()):
                    #     min = 1000000000
                    #     for m in range(collection3.__len__()):
                    #         if l != m:
                    #             x = collection3[l]['position_x'] - collection3[m]['position_x']
                    #             x *= x
                    #             y = collection3[l]['position_y'] - collection3[m]['position_y']
                    #             y *= y
                    #             dist = math.sqrt(x + y)
                    #             if dist < min:
                    #                 min = dist
                    #     min_dict_1[i] = min
                    # print(min_dict_1)
                    # for l in min_dict_1.keys():
                    #     for m in min_dict_2.keys():
                    #         if min_dict_1[l] == min_dict_2[m]:
                    #             print(l, "--------------", m)
                    # k.append(j)
                    # break
    return matched
